...
/** Minimal valid installed catalog covering the four Free groups `init` emits. */
function validCatalog(): string {
  const groups = ["moe-outline", "moe-solid", "moe-lite-outline", "moe-colored"].map((id) => ({
    id,
    type: id === "moe-colored" ? "mixed" : "outline",
    tiers: ["free", "pro"],
    formats: ["svg"],
    imageSizes: [],
  }));
  return JSON.stringify({
    ...
    styleGroups: groups,
    icons: [{ id: "ui-search", prefix: "ui", label: "Search", aliases: [], availableIn: groups.map((group) => group.id) }],
  });
}
